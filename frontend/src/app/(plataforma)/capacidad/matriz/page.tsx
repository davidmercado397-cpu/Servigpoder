...
export default function MatrizPage() {
  const puedeEditar = usePermiso("capacidad.matriz.gestionar");
  ...
  const cargarPeriodos = useCallback(async () => {
    const lista = await api<Periodo[]>("/capacidad/matriz/periodos");
    setPeriodos(lista);
  ...
  async function proyectar() {
    ...
    await accion(async () => {
      const nuevo = await api<Periodo>(`/capacidad/matriz/periodos/${periodo.id}/proyectar`, { method: "POST", json: { copiar_excepciones: copiar } });
  ...
  async function cambiarFestivos(mp: MatrizPuesto, incluye: boolean) {
    ...
    try {
      const r = await api<MatrizPuesto>(`/capacidad/matriz/puestos/${mp.id}`, { method: "PATCH", json: { incluye_festivos: incluye } });
...
function Importar({ onListo }: { onListo: (periodoId: number) => void }) {
  ...
  async function enviar(e: React.FormEvent) {
    ...
    setCargando(true);
    try {
      const r = await subirArchivo<{ periodo_id: number; avisos: string[] }>("/capacidad/matriz/importar", archivo, { anio: String(anio), mes: String(mes) });
...
function Editor({ mp, editable, onGuardado }: { mp: MatrizPuesto; editable: boolean; onGuardado: (mp: MatrizPuesto) => void }) {
  ...
  const cargarDias = useCallback(async () => setDias(await api<DiaRequerido[]>(`/capacidad/matriz/puestos/${mp.id}/requerimiento`)), [mp.id]);
  useEffect(() => {
    cargarDias();
  }, [cargarDias]);
  ...
  async function guardar(extra: Record<string, unknown> = {}) {
    setError("");
    try {
      const r = await api<MatrizPuesto>(`/capacidad/matriz/puestos/${mp.id}`, {
        method: "PATCH",
  ...
  async function agregarExcepcion(e: React.FormEvent) {
    ...
    setError("");
    try {
      const nueva = await api<Excepcion>(`/capacidad/matriz/puestos/${mp.id}/excepciones`, {
        method: "POST",
        ...
        <ul className="space-y-1 text-sm">
          ...
          {mp.excepciones.map((e) => (
            <li key={e.id} className="flex items-center justify-between rounded bg-slate-50 px-2 py-1">
              ...
              {editable && (
                <button className="text-red-600" onClick={async () => { await api(`/capacidad/matriz/excepciones/${e.id}`, { method: "DELETE" }); onGuardado({ ...mp, excepciones: mp.excepciones.filter((x) => x.id !== e.id) }); cargarDias(); }}>✕</button>
              )}
            </li>
          ))}
        </ul>

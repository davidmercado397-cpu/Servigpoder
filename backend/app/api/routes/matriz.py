...
from app.models import MatrizExcepcion, MatrizFranja, MatrizPeriodo, MatrizPuesto, Puesto, Ubicacion, Usuario
from app.models.matriz import CERRADO
...
from app.services import matriz as svc
...
def listar_periodos(db: DbSession, _=Depends(require("matriz.ver"))):
    stats = {
        pid: (n, h or 0, r or 0)
...
async def importar(request: Request, db: DbSession, archivo: UploadFile = File(...),
                   ...
                   actual: Usuario = Depends(require("matriz.gestionar"))):
...
def eliminar_excepcion(exc_id: int, request: Request, db: DbSession, actual: Usuario = Depends(require("matriz.gestionar"))):
...
def requerimiento(mp_id: int, db: DbSession, _=Depends(require("matriz.ver"))):
...
def festivos(anio: int = Query(..., ge=2020, le=2100), mes: int = Query(..., ge=1, le=12), _=Depends(require("matriz.ver"))):

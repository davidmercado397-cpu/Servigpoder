...
from app.apps.capacidad.models.analisis import HUECO, MIXTO
from app.apps.capacidad.models.cubrimientos import PENDIENTE
from app.apps.capacidad.models.matriz import BORRADOR
from app.apps.capacidad.services.matriz import siguiente_mes
...
def evaluar(db: Session, fecha: date | None = None) -> list[dict]:
    ...
    if periodo is None:
        alertas.append(Alerta(CRITICA, "No hay matriz comercial para este mes",
                              "Sin matriz no se puede medir la cobertura. Proyecte la del mes anterior o impórtela.", "/capacidad/matriz"))
    elif periodo.estado == BORRADOR:
        alertas.append(Alerta(INFO, "La matriz de este mes sigue en borrador", "Revísela y apruébela.", "/capacidad/matriz"))
    ...
    if dias_restantes <= prm["dias_aviso_proyeccion"] and not db.scalar(
        ...
        alertas.append(Alerta(ADVERTENCIA, "Falta proyectar la matriz del mes siguiente",
                              f"Quedan {dias_restantes} días del mes y aún no existe la matriz de {mes_sig:02d}/{anio_sig}.", "/capacidad/matriz"))

    # Programación
    if carga is None:
        alertas.append(Alerta(CRITICA, "No hay programación cargada para este mes",
                              "Suba el reporte de asignación de SIESA.", "/capacidad/programacion"))
    else:
        ...
        if sin_puesto or aproximadas:
            alertas.append(Alerta(ADVERTENCIA, "Puestos de SIESA por aclarar",
                                  ...
                                  "/capacidad/maestros", clave="por_aclarar", valor=sin_puesto + aproximadas))
    ...
    if carga and periodo and analisis is None:
        alertas.append(Alerta(ADVERTENCIA, "La cobertura del mes no está calculada", "Calcule el análisis.", "/capacidad/cobertura"))
    ...
    if analisis:
        ...
        if periodo_mod and periodo_mod > generado:
            alertas.append(Alerta(ADVERTENCIA, "El análisis está desactualizado",
                                  "La matriz cambió después del último cálculo. Recalcule la cobertura.", "/capacidad/cobertura"))
        ...
        if huecos:
            alertas.append(Alerta(
                ...
                "Horas vendidas sin nadie programado en los próximos días.", "/capacidad/cobertura",
        ...
        if pendientes:
            alertas.append(Alerta(ADVERTENCIA, f"{len(pendientes)} cubrimientos pendientes de revisión de nómina",
                                  f"{sum(float(c.horas) for c in pendientes):,.0f} horas sin justificación automática.", "/capacidad/cubrimientos",

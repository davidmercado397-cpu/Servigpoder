...
export default function Inicio() {
  ...
  useEffect(() => {
    ...
    apiEnvelope<AlertaT[]>("/alertas").then((r) => {
      setAlertas(r.data ?? []);
      setFecha(String(r.meta.extra?.fecha ?? ""));
    });
    api<MesDisponible[]>("/analisis/meses").then((m) => {
      const conAnalisis = m.find((x) => x.analisis_id);
      if (conAnalisis?.analisis_id) api<Analisis>(`/analisis/${conAnalisis.analisis_id}`).then(setAnalisis);
    });
  }, [puedeAnalisis]);
    ...
    <div className="max-w-5xl space-y-6">
      ...
      {r && (
        <section className="grid gap-3 md:grid-cols-4">
          <Link href="/cobertura" className="tarjeta p-4 hover:bg-slate-50">
          ...
          <Link href="/cubrimientos" className="tarjeta p-4 hover:bg-slate-50">
                    ...
                    <tbody>
                      {a.items.map((it, j) => (
                        <tr key={j}>
                          ...
                          <td>
                            {analisis ? (
                              <Link className="font-mono text-marca-700 hover:underline" href={`/cobertura/${it.puesto_id}?analisis=${analisis.id}`}>{String(it.puesto)}</Link>
                            ) : String(it.puesto)}
                          </td>

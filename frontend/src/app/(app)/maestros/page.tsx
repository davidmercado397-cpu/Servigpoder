...
function PorAclararTab() {
  const puedeEditar = usePermiso("maestros.gestionar");
  const [items, setItems] = useState<PorAclarar[]>([]);
  const [puestos, setPuestos] = useState<Puesto[]>([]);
  const [msg, setMsg] = useState<{ tipo: "ok" | "error"; texto: string } | null>(null);

  const cargar = useCallback(async () => {
    setItems(await api<PorAclarar[]>("/maestros/por-aclarar"));
  }, []);

  useEffect(() => {
    cargar();
    api<Puesto[]>("/maestros/puestos").then(setPuestos);
  }, [cargar]);

  async function asignar(codigo: string, puestoId: number) {
    try {
      await api("/maestros/equivalencias", { method: "PUT", json: { codigo_siesa: codigo, puesto_id: puestoId } });
...
function PuestosTab() {
  ...
  const cargar = useCallback(async () => {
    setPuestos(await api<Puesto[]>(`/maestros/puestos?q=${encodeURIComponent(q)}`));
  }, [q]);
  ...
  async function cambiar(p: Puesto, campo: "excluido" | "activo", valor: boolean) {
    await api(`/maestros/puestos/${p.id}`, { method: "PATCH", json: { [campo]: valor } });
    cargar();
  }
...
function TurnosTab() {
  ...
  const [msg, setMsg] = useState<{ tipo: "ok" | "error"; texto: string } | null>(null);
  const cargar = useCallback(async () => setTurnos(await api<Turno[]>("/catalogos/turnos")), []);
  useEffect(() => {
    cargar();
  }, [cargar]);
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        {puedeEditar && (
          <SubirArchivo
            ...
            onArchivo={async (f) => {
              try {
                const r = await subirArchivo<{ turnos: number }>("/catalogos/turnos/importar", f);
...
function NovedadesTab() {
  ...
  const cargar = useCallback(async () => setItems(await api<Novedad[]>("/catalogos/novedades")), []);
  useEffect(() => {
    cargar();
  }, [cargar]);
  ...
  async function guardar(n: Novedad) {
    setError("");
    try {
      await api("/catalogos/novedades", { method: "PUT", json: n });
      cargar();
    } catch (e) {

...
class Usuario(Base):
    ...
    # Contraseña temporal (creada o restablecida por un administrador): debe cambiarla al entrar
    debe_cambiar_password: Mapped[bool] = mapped_column(Boolean, default=False, server_default="false")
    password_actualizado_en: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    # Verificación en dos pasos (TOTP). El secreto se guarda cifrado.
    mfa_activo: Mapped[bool] = mapped_column(Boolean, default=False, server_default="false")
    mfa_secreto: Mapped[str | None] = mapped_column(String(300), nullable=True)
    mfa_ultimo_paso: Mapped[int | None] = mapped_column(Integer, nullable=True)  # evita reutilizar un código

...
export default function CoberturaPage() {
  ...
  useEffect(() => {
    setAnalisis(null);
    if (mes?.analisis_id) api<Analisis>(`/analisis/${mes.analisis_id}`).then(setAnalisis);
  }, [mes]);
  ...
  const cargarPuestos = useCallback(async () => {
    ...
    setPuestos(await api<PuestoAnalisis[]>(`/analisis/${analisis.id}/puestos?${p}`));
  ...
  async function calcular() {
    ...
    setCalculando(true);
    try {
      const a = await api<Analisis>("/analisis", { method: "POST", json: { anio: mes.anio, mes: mes.mes } });
      setAnalisis(a);
  ...
  return (
    <div className="max-w-7xl space-y-5">
      <Titulo
        accion={
          <div className="flex flex-wrap items-center gap-2">
            ...
            {analisis && <a className="btn-secundario" href={`/api/analisis/${analisis.id}/exportar`}>Exportar a Excel</a>}
          </div>
        }
      >
      ...
      {meses.length === 0 && <Alerta tipo="info">Aún no hay programación cargada. Súbala en <Link className="underline" href="/programacion">Programación SIESA</Link>.</Alerta>}
      {mes && !mes.tiene_matriz && <Alerta>No existe matriz comercial para {nombreMes(mes.anio, mes.mes)}. Proyéctela o impórtela en <Link className="underline" href="/matriz">Matriz comercial</Link>.</Alerta>}
        ...
        <>
          ...
          <section className="grid gap-4 lg:grid-cols-[18rem_1fr]">
            ...
            <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
              ...
              <Tile label="Filas sin puesto" valor={num(r.filas_sin_puesto)} nota="códigos SIESA por aclarar" href="/maestros" />
            </div>
          </section>
                    ...
                    return (
                      <tr key={p.puesto.id} className="hover:bg-slate-50">
                        <td className="font-mono font-semibold">
                          <Link className="text-marca-700 hover:underline" href={`/cobertura/${p.puesto.id}?analisis=${analisis!.id}`}>{p.puesto.codigo}</Link>
                        </td>
                        <td>

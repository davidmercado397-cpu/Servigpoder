...
from app.apps.capacidad.models import Auditoria, Puesto, Usuario
from app.apps.capacidad.services import asistente as svc
from app.apps.capacidad.services.alertas import ZONA
...
def _contexto_pantalla(db: DbSession, pantalla: str | None) -> str | None:
    ...
    nombres = {"/capacidad/cobertura": "el tablero de cobertura", "/capacidad/cubrimientos": "la bandeja de cubrimientos de nómina",
...
def estado(db: DbSession, actual: Usuario = Depends(require("capacidad.asistente.usar"))):
    s = get_settings()
...
def preguntar(data: PreguntaIn, request: Request, db: DbSession, actual: Usuario = Depends(require("capacidad.asistente.usar"))):
    s = get_settings()

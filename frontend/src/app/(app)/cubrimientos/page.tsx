...
export default function CubrimientosPage() {
  const puedeAprobar = usePermiso("cubrimientos.aprobar");
  ...
  const cargar = useCallback(async () => {
    ...
    const r = await apiEnvelope<Cubrimiento[]>(`/cubrimientos/${analisisId}?${p}`);
    setLista(r.data ?? []);
  ...
  async function decidir(nuevo: "aprobado" | "rechazado" | "pendiente", ids: number[]) {
    ...
    setMsg(null);
    try {
      await api(`/cubrimientos/${analisisId}/decidir`, { method: "POST", json: { ids, estado: nuevo, comentario } });
  ...
  return (
    <div className="max-w-7xl space-y-4">
      ...
      {cargado && !analisisId && <Alerta>Este mes no tiene análisis de cobertura. Calcúlelo en <Link className="underline" href="/cobertura">Cobertura</Link>.</Alerta>}
              ...
              <tbody>
                ...
                {lista.map((c) => (
                  <tr key={c.id} className={sel.has(c.id) ? "bg-marca-50" : ""}>
                    ...
                    <td>
                      <Link className="font-mono font-semibold text-marca-700 hover:underline" href={`/cobertura/${c.puesto.id}?analisis=${analisisId}`}>{c.puesto.codigo}</Link>

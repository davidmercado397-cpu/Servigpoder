...
async def subir(request: Request, db: DbSession, archivo: UploadFile = File(...),
                actual: Usuario = Depends(require("programacion.cargar"))):
...
def listar(db: DbSession, _=Depends(require("analisis.ver"))):
...
def detalle(carga_id: int, db: DbSession, _=Depends(require("analisis.ver"))):

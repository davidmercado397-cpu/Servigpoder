...
from app.models import ProgramacionCarga, ProgramacionFila, Puesto, PuestoEquivalencia, Ubicacion, Usuario
from app.schemas.f1 import EquivalenciaIn, EquivalenciaOut, PorAclarar, PuestoEditar, PuestoOut
from app.services.codigos import limpiar

router = APIRouter(prefix="/maestros", tags=["maestros"])


@router.get("/puestos", response_model=ApiResponse[list[PuestoOut]])
def listar_puestos(db: DbSession, q: str = Query("", max_length=100), _=Depends(require("maestros.ver"))):
...
def editar_puesto(puesto_id: int, data: PuestoEditar, request: Request, db: DbSession,
                  actual: Usuario = Depends(require("maestros.gestionar"))):
...
def listar_equivalencias(db: DbSession, _=Depends(require("maestros.ver"))):
...
def por_aclarar(db: DbSession, _=Depends(require("maestros.ver"))):

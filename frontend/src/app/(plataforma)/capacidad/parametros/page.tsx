...
export default function ParametrosPage() {
  const puedeEditar = usePermiso("capacidad.parametros.gestionar");
  const [lista, setLista] = useState<Parametro[]>([]);
  const [valores, setValores] = useState<Record<string, string>>({});
  const [msg, setMsg] = useState<{ tipo: "ok" | "error"; texto: string } | null>(null);

  useEffect(() => {
    api<Parametro[]>("/capacidad/parametros").then((l) => {
      setLista(l);
  ...
  async function guardar(clave: string) {
    setMsg(null);
    try {
      await api(`/capacidad/parametros/${clave}`, { method: "PUT", json: { valor: valores[clave] } });

...
export default function ProgramacionPage() {
  const puedeCargar = usePermiso("capacidad.programacion.cargar");
          ...
          puedeCargar && (
            <SubirArchivo
              ...
              onArchivo={async (f) => {
                setError("");
                try {
                  const c = await subirArchivo<Carga>("/capacidad/programacion/cargas", f);
                  setActual(c);
                  cargar();
                } catch (e) {
    ...
    <section className="tarjeta space-y-4 p-5">
      ...
      {r.puestos_sin_equivalencia.length > 0 ? (
        <Alerta>
          ...
          <Link href="/capacidad/maestros" className="font-semibold underline">Aclararlos en Maestros</Link>
        </Alerta>
      ) : (
        <Alerta tipo="ok">Todos los puestos se cruzaron con la matriz.</Alerta>
      )}
      <Link href="/capacidad/cobertura" className="btn-primario">Ver cobertura de este mes →</Link>
    </section>
  );
}
